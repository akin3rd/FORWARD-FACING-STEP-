...
!    Enddo
 ...
 !   Xs = (xa+xb)/2  + GauPoints(j)*(xb-xa)/2
   ...
   OPEN(111,FILE = 'movie_VEN_Rus.dat')
 ...
 ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   Subroutine ComputeConvergenceError
    ...
    integer i
     ...
     open(1,file='errors_VEN_Rus.txt')
...
! ***************** Linear & TVD of Barth  *************************************************
  Subroutine ComputeReconstructionBarth_i(Rkstage,i,schemetype,ExtValuesI)
   ...
   Real(8) ExtValuesI(4,4,4)
   real::DMIN,DPLUS,SFD,D2,KAPPA_VEN,DELTA_X,EPSI2,EPSI_ZERO
...
!         End select
	  ...
	  ! compute psi (Limiters of Barth & Jesperson OR Venkatakrishnan)
      Do L=1,MeshElements(i)%NumberOfSides
	   ...
	   D2=(Vert(L)-Va);KAPPA_Ven=10.0D0 ;DELTA_X=MeshElements(i)%NumberofSides
	   EPSI_ZERO=1E-16
	   ...
	   DPLUS= (umax-va)
                ...
                ELSE
                        IF (D2.GT. 0.0D0)THEN
                       psi(L)= 1/(sign(1.0,d2)*(abs(d2)+epsi_zero)) *(((DPLUS**2+epsi2**2)*D2+2*D2**2+DPLUS)/(dplus**2+2*d2**2+dplus*d2+epsi2**2))
					    
                        else
                       
                      psi(L)= (1/(sign(1.0,d2)*(abs(d2)+epsi_zero)) *(((DMIN**2+epsi2**2)*D2+2*D2**2+DMIN)/(dmin**2+2*d2**2+dmin*d2+epsi2**2)))
...
!   CALL ComputeReconstructionVertexes
    ...
    open(1,file='rho_VEN_Rus.dat')
    write(1,*) 'TITLE = "rho""'
    write(1,*)' VARIABLES = "x" "y" "rho"'
   
    open(2,file='p_VEN_Rus.dat')
    write(2,*) 'TITLE = "p""'
    write(2,*)' VARIABLES = "x" "y" "p"'

    open(3,file='vel_VEN_Rus.dat')
    write(3,*) 'TITLE = "vel""'
    write(3,*)' VARIABLES = "x" "y" "vel"'

    open(4,file='stream_VEN_Rus.dat')
...
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  Subroutine WriteInfo
   ...
   file_id = 555
   open(file_id,file='info_VEN_Rus.txt')
...
!  compute the Godunov flux with the HLLC Riemann solver
   SUBROUTINE HLLC_FLUX(CDL,CDR,FHLLC)
      ...
      !PL = PVL(4)
	!  CL = SQRT(Gamma*PL/DL)
        !EL= (PL/(Gamma-1.0))*0.5*DL*(UL**2+VL**2)
      ...
      !PR = PVR(4)
	!  CR = SQRT(Gamma*PR/DR)
     !ER= (PR/(Gamma-1.0))*0.5*DR*(UR**2+VR**2)

      ul = cdl(2)/cdl(1)
      vl= cdl(3)/cdl(1)
      el=cdl(4)
      dl=cdl(1)
      pl = (gamma-1)*(el - 0.5*dl*ul**2)

      ur = cdr(2)/cdr(1)
      vr = cdr(3)/cdr(1)
      er=cdr(4)
      dr=cdr(1)
      pr = (gamma-1)*(er - 0.5*dr*ur**2)

   !u and v are velocites in x and y axes respectively, e is specific energy, d is density and p is pressure.

     ! Estimates for wave speeds are already calculated
     

    ! Calculate the contact wave speed
    !SM = (CDR * UR - CDL * UL + FDL(1) - FDR(1)) / (CDR - CDL)
    SS=(pr-pl+(dl*UL*(SL-UL))-(dr*UR*(SR-UR)))/((dl*(SL-UL))-(dr*(SR-UR)))

        CSL(1)= dl*((SL-UL)/(SL-SS))*1
	CSL(2)= dl*((SL-UL)/(SL-SS))*SS
        CSL(3)= dl*((SL-UL)/(SL-SS))*VL
	CSL(4)= dl*((SL-UL)/(SL-SS))*((EL/DL)+(SS-UL)*(SS+(PL/(DL*(SL-UL)))))

	CSR(1)= dr*((SR-UR)/(SR-SS))*1
	CSR(2)= dr*((SR-UR)/(SR-SS))*SS
        CSR(3)= dr*((SR-UR)/(SR-SS))*VR
	CSR(4)= dr*((SR-UR)/(SR-SS))*((ER/DR)+(SS-UR)*(SS+(PR/(DR*(SR-UR))))) 
	 ...
	 if (0.0 .le. SL) then
		    FHLLC = FDL
		  else if (SL <= 0.0 .and. SS >= 0.0) then
		    FHLLC = FDL+(SL*(CSL-CDL))
                  else if (SS <= 0.0 .and. SR >= 0.0) then
	            FHLLC = FDR+(SR*(CSR-CDR))
		  else if (0.0 .ge. SR) then
		    FHLLC = FDR
	      end if  
